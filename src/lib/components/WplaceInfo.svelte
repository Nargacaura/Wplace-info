<script lang="ts">
	...
	import { backendInfo, formatCoordinate, formatTime, getLocalStorageLocale } from '$lib';
</script>
...
<div class="my-8 flex flex-col items-center">
	...
	<div class="grid w-[90%] grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <fieldset
            class="mb-4 rounded border border-gray-300 p-4 text-center"
        >
		...
		<fieldset class="rounded border border-gray-300 p-4 text-center">
			...
			<ul class="list-inside list-disc text-left">
				...
				<li>
					...
					{Boolean($backendInfo!.allianceId) ? $_('boolean.yes') : $_('boolean.no')}
				</li>
		...
		<fieldset class="rounded border border-gray-300 p-4 text-center">
			...
			<ul class="list-inside list-disc text-left">
				...
				<li>
					...
					{formatTime(($backendInfo!.charges.max * $backendInfo!.charges.cooldownMs) - ($backendInfo!.charges.count * $backendInfo!.charges.cooldownMs))}
				</li>
			</ul>
		</fieldset>
        ...
        <!-- Favorites fieldset -->
        <fieldset class="rounded border border-gray-300 p-4 text-center">
            <legend class="text-xl">{$_('data.favorites')} ({$backendInfo!.favoriteLocations.length} / {$backendInfo!.maxFavoriteLocations})</legend>
            <ul class="list-inside list-disc text-left">
                {#if $backendInfo!.favoriteLocations.length > 0}
                    {#each $backendInfo!.favoriteLocations as location}
                        <li>
                            <a href="https://wplace.live/?lat={location.latitude}&lng={location.longitude}&zoom=14" target="_blank" rel="noopener noreferrer" class="text-blue-500">
